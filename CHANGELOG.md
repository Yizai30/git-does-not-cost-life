# Changelog

All notable changes to the git-submit project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

### Added
- Initial release of git-submit
- Infinite retry logic with exponential backoff and jitter
- Multi-channel notifications (email, desktop, webhook)
- State persistence for crash safety and resume capability
- Structured JSON logging with daily rotation
- Cross-platform support (Windows, macOS, Linux)
- CLI with comprehensive configuration options
