#!/usr/bin/env python3
"""
git-submit CLI - FULL WORKING VERSION

This version includes all commands: config, status, cleanup, history, and PUSH!
"""
...
def execute_push(remote, branch, all_branches=False, force=False, dry_run=False):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"\n✗ Push failed (exit code {result.returncode})")
            if result.stderr:
                print(f"Error: {result.stderr}")
            return False
    except Exception as e:
        print(f"\n✗ Error executing git push: {e}")
        return False
    ...
    try:
        ...
        if args.command == "config":
            ...
            for attempt in range(1, max_retries + 1):
                ...
                if success:
                    if not args.dry_run:
                        print(f"\n✓ Push successful on attempt {attempt}!")
